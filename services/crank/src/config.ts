...
export function getConfig(env: NodeJS.ProcessEnv): Config {
  const network = (env.SOLANA_NETWORK || 'devnet') as 'mainnet' | 'devnet';
  const rpcUrl = env.RPC_URL || 'https://api.devnet.solana.com';
  const programId = env.VERIBET_PROGRAM_ID || env.PROGRAM_ID || '2Syq46YQQ4iGbCouFYxjeHEcABScMd669NAK5XrxZFWG';
  ...
  if (!programId) {
    throw new Error('Program ID is not set in environment variables (VERIBET_PROGRAM_ID)');
  }
  if (!rpcUrl) {
    throw new Error('RPC URL is not set (RPC_URL)');
  }
